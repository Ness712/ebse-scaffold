...
> **Version** : 1.5
> **Source** : ebse-scaffold domaine `ai-collaboration` (33 PICOCs, Kitchenham 2007)
...
3. **Build** — verifier que le build passe. **Si Dockerfile ou docker-compose modifie** → `docker build -t test-local .` puis `docker run --rm --entrypoint sh test-local -c 'ls /app/dist || ls /dist'` avant tout push (boucle de feedback : CI post-push = 10-20 min par iteration Dockerfile — PICOC containerization)
